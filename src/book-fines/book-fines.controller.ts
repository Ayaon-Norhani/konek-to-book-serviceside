...
export class BookFinesController {
  ...
  @ApiOperation({ summary: 'Get BookFines by ID', operationId: 'GetBookFine' })
  @ApiResponse({ status: 200, type: BookFinesDto })
  @Get(':BookFinesID')
  async findOne(
    @Param('BookFinesID') BookFines_ID: number,
  ): Promise<BookFinesDto> {
    return this.bookfinesService.findOne(BookFines_ID);
  }

  @ApiOperation({
    summary: 'Update BookFines by ID',
    operationId: 'UpdateBookFine',
  })
  @Put(':BookFinesID')
  async update(
    @Param('BookFinesID') BookFines_ID: number,
    @Body() job: BookFinesDto,
  ) {
